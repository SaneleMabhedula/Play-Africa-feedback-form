...
def show_home() -> None:
    ...
    if not st.session_state.authenticated or st.session_state.role == "visitor":
        ...
        with st.container():
            st.markdown("<h3 style='text-align: center;'>Scan this QR code to access the feedback form</h3>", 
                       unsafe_allow_html=True)
            show_qr_code(qr_url)
            
            st.markdown("""
            <div style='text-align: center; margin-top: 20px;'>
                <p>Print this QR code and display it in your facility for easy access.</p>
            </div>
            """, unsafe_allow_html=True)
