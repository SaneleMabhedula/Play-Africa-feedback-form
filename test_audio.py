...
def audio_recorder():
    ...
    async function startRecording() {
        try {
            ...
            // Send message to Streamlit when recording starts
        ...
        return new Promise(resolve => {
            recorder.onstop = async () => {
                try {
                    ...
                    document.getElementById("status").innerText = "Error processing recording";
    ...
    window.addEventListener('message', (event) => {
        if (event.data.type === 'triggerStopRecording') {
            stopRecording();
        }
    });
    </script>
    ...
    <div style="margin: 10px 0;">
        <button onclick="startRecording()" style="padding: 8px 16px; margin-right: 10px; background-color: #2E86AB; color: white; border: none; border-radius: 4px; cursor: pointer;">
            🎤 Start Recording
        </button>
        <button onclick="stopRecording()" style="padding: 8px 16px; background-color: #F18F01; color: white; border: none; border-radius: 4px; cursor: pointer;">
            ⏹️ Stop Recording
        </button>
        <p id="status" style="margin-top: 10px; font-size: 14px; color: #555;">Ready to record</p>
    </div>
    """
            ...
            if submitted:
                if not all(required_fields):
                    ...
                    entry = {
                        ...
                        "audio_file": st.session_state.audio_file if st.session_state.get('audio_file') else "",
...
def main() -> None:
    ...
    # JavaScript to handle audio data from the recorder component
    st.markdown("""
    <script>
    // Listen for messages from the audio recorder iframe
